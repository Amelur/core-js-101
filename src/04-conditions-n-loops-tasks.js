...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const leftBound = isStartIncluded ? '[' : '(';
  const rightBound = isEndIncluded ? ']' : ')';
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (open.includes(char)) {
      ...
      const expectedClosingBracket = open[close.indexOf(char)];
      if (lastOpeningBracket !== expectedClosingBracket) {
        return false;
      }
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  const sortedPaths = pathes.slice().sort();
  const firstPath = sortedPaths[0];
  const lastPath = sortedPaths[sortedPaths.length - 1];

  let commonPrefixLength = 0;
  while (
    commonPrefixLength < firstPath.length
    && firstPath[commonPrefixLength] === lastPath[commonPrefixLength]
  ) {
    commonPrefixLength += 1;
  }

  const commonPath = firstPath.substring(0, commonPrefixLength);

  const lastSlashIndex = commonPath.lastIndexOf('/');
  return commonPath.substring(0, lastSlashIndex + 1);
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < winningCombinations.length; i += 1) {
    ...
    const cell1 = position[c1[0]][c1[1]];
    const cell2 = position[c2[0]][c2[1]];
    const cell3 = position[c3[0]][c3[1]];
